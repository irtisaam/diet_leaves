...
export default function EditProductPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    is_on_sale: false,
    ingredients: '',
    servings_per_container: null as number | null,
      ...
      if (res.ok) {
        ...
        if (product) {
          ...
          setFormData({
            ...
            nutritional_info: product.nutritional_info || defaultNutritionalInfo
          })
          setImages(product.images || [])
          setNutritionalImage(extractNutritionalImage(product.description))

          // Fetch existing inventory mappings
          try {
            const mapRes = await fetch(`${API_URL}/api/admin/inventory/product-mappings?product_id=${productId}`)
            if (mapRes.ok) {
              const mapData = await mapRes.json()
              if (Array.isArray(mapData)) {
                setProductMappings(mapData.map((m: any) => ({
                  id: m.id,
  ...
  const extractDescription = (desc: string | null): string => {
    if (!desc) return ''
    return desc.replace(/\n\n\[nutritional_image:.*?\]/g, '')
  }

  const extractNutritionalImage = (desc: string | null): string => {
    if (!desc) return ''
    const match = desc.match(/\[nutritional_image:(.*?)\]/)
  ...
  const removeImage = async (index: number) => {
    ...
    if (img.id) {
      try {
        await fetch(`${API_URL}/api/admin/products/${productId}/images/${img.id}`, {
          method: 'DELETE'
        })
      } catch (error) {
  ...
  const setPrimaryImage = async (index: number) => {
    ...
    if (img.id) {
      try {
        await fetch(`${API_URL}/api/admin/products/${productId}/images/${img.id}/primary`, {
          method: 'PUT'
        })
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        try {
          await fetch(`${API_URL}/api/admin/inventory/product-mappings/bulk/${productId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
              ...
              {Object.entries(formData.nutritional_info).map(([key, value]) => (
                <div key={key}>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.1"
                    className="w-full px-3 py-2 bg-black border border-zinc-700 rounded-lg text-white text-sm focus:outline-none focus:border-white transition-colors"
                  />
                </div>
              ))}
            </div>
